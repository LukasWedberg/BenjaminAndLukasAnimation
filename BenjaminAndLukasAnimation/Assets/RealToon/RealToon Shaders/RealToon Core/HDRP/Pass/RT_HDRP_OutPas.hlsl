...
Varyings LitPassVertex(Attributes input)
{
	...
	_OEM = lerp(_normloc, SafeNormalize(input.positionOS.xyz), _OutlineExtrudeMethod) * _OutResi;
	...
	//RT_PA
	#if N_F_PA_ON
		output.positionCS = mul(RT_PA(-TransformWorldToView(output.positionWS).z), float4( (input.positionOS.xyz + _OutlineOffset.xyz * 0.01) + _OEM * RTD_OL, 1.0) ) + (float4(0,0,_ObjePosiZCS,0.0)* 0.0001);
	#else
        output.positionCS = TransformWorldToHClip(TransformObjectToWorld((input.positionOS.xyz + _OutlineOffset.xyz * 0.01) + _OEM * RTD_OL)) + (float4(0, 0, _ObjePosiZCS, 0.0) * 0.0001);
	#endif
	//==
