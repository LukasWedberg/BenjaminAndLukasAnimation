...
Varyings LitPassVertex(Attributes input)
{
...
#endif
	{
		...
		//RT_SE
		#if N_F_SE_ON
			input.positionOS = RT_SE(output.positionWS);
		#endif
		//==
		...
		//RT_PA
        #if N_F_PA_ON
			output.positionCS = mul(RT_PA(-TransformWorldToView(output.positionWS).z), float4(input.positionOS.xyz, 1.0)) + (float4(0,0,_ObjePosiZCS,0.0)* 0.0001);
		#else
            output.positionCS = TransformWorldToHClip(output.positionWS) + (float4(0, 0, _ObjePosiZCS, 0.0) * 0.0001);
		#endif
		//==
		...
		//RT_SON	
		#if N_F_SON_ON
			output.smoNorm = GetAbsolutePositionWS(output.positionWS);
		#endif
		//==	
...
)
{
	...
	//RT_DC	
        RT_DC(posInput, isFrontFace, _MainTex_var, normalDirection); //cc
	//==
	...
	if (LIGHTFEATUREFLAGS_DIRECTIONAL)
	{
		...
		for (i = 0; i < _DirectionalLightCount; ++i)
		{
			...
			if (IsMatchingLightLayer(light.lightLayers, builtinData.renderingLayers))
			{
				...
				float RTD_GLO; 
				float3 RTD_GLO_COL;
                RT_GLO(input.uv, RTD_VD_Cal, halfDirection, input.bitangentWS, normalDirection, viewDirection, input.positionWS.xyz, DirSpecDim, RTD_GLO, RTD_GLO_COL);
				...
				float RTD_ST_IS;
				float3 RTD_ST_LAF;
				float RTD_ST = RT_ST(input.uv, input.positionWS.xyz, normalDirection, RTD_NDOTL, attenuation, lightDirection, RTD_LVLC, RTD_PT_COL, lig_col_int, RTD_SCT, RTD_OSC, RTD_PT, RTD_SHAT_COL, RTD_STIAL, RTD_ST_IS, RTD_ST_LAF);
				//==
            ...
            else
            {
				...
				float3 RTD_MCIALO = lerp(RTD_TEX_COL , lerp( lerp( (RTD_TEX_COL * _MainColor.rgb), (RTD_TEX_COL + _MainColor.rgb), _SPECMODE) , _MainTex_var.rgb * MCapOutP * _RTD_MVCOL * 0.7 , clamp((0*1.0),0.0,1.0) ) , _MCIALO );
			
		
				//RT_RL
				float3 RTD_RL_LARL_OO;
				float RTD_RL_MAIN;
				float RTD_RL_CHE_1 = RT_RL(viewDirection, normalDirection, 0, sceneUVs.xy, input.positionCS, RTD_RL_LARL_OO, RTD_RL_MAIN);
				//==
		
			
				//RT_RELGI_SUB1
				float ref_int_val;
				float3 RTD_SL_OFF_OTHERS = RT_RELGI_SUB1(posInput, viewReflectDirection, viewDirection, RTD_GI_FS_OO, 0, RTD_MCIALO, 1, RTD_RT_GI_Sha_FO, ref_int_val, (float3)0.0, false);
				//==
		
		
				//RT_R
				float3 RTD_R = RT_R( input.uv, viewDirection, normalDirection, input.positionWS.xyz, EnvRef, RTD_TEX_COL, 0 );
				//==
	...
	if (LIGHTFEATUREFLAGS_PUNCTUAL)
	{
		...
		for (i = 0; i < i_en; ++i)
		{
			...
			float RTD_GLO; 
			float3 RTD_GLO_COL;
			RT_GLO( input.uv, RTD_VD_Cal, halfDirection, input.bitangentWS, normalDirection, viewDirection, input.positionWS.xyz, Plight.specularDimmer, RTD_GLO, RTD_GLO_COL );
			float3 RTD_GLO_OTHERS = RTD_GLO;
			//==
			...
			float RTD_ST_IS;
			float3 RTD_ST_LAF;
			float RTD_ST = RT_ST ( input.uv, input.positionWS.xyz, normalDirection, RTD_NDOTL, lightfos, lightDirection, RTD_LVLC, RTD_PT_COL, lig_col_int, RTD_SCT, RTD_OSC, RTD_PT, RTD_SHAT_COL, RTD_STIAL, RTD_ST_IS, RTD_ST_LAF );
			//==
				...
				if (NALC > 0)
				{
					...
					while (i <= last )
					{
						...
						if(IsMatchingLightLayer(Alight.lightLayers, builtinData.renderingLayers))
						{
							...
							float RTD_GLO; 
							float3 RTD_GLO_COL;
							RT_GLO( input.uv, RTD_VD_Cal, halfDirection, input.bitangentWS, normalDirection, viewDirection, input.positionWS.xyz, Alight.specularDimmer, RTD_GLO, RTD_GLO_COL ); 
							//==
...
float4 finalRGBA=float4(RTD_CA*RTD_SON_CHE_1,Trans_Val);//FOP
	...
	//
	#ifdef UNITY_VIRTUAL_TEXTURING
		float vtAlphaValue = Trans_Val;
		outVTFeedback = PackVTFeedbackWithAlpha(builtinData.vtPackedFeedback, input.positionSS.xy, vtAlphaValue);
		outVTFeedback.rgb *= outVTFeedback.a;
	#endif
	//
}
