...
void RT_DL( LightLoopContext context , DirectionalLightData light , out float4 DirLigCol , out float3 DirLigDir , out float DirSha , out float DirLigDim , out float DirDifDim , out float DirSpecDim , PositionInputs posInput , BuiltinData builtinData )
{
	...
	float4 DLCol = float4(light.color * GetCurrentExposureMultiplier(), 1.0);
	#ifdef N_F_LLI_ON
		DirLigCol = clamp(DLCol,_LLI_Min,_LLI_Max);
	#else
		DirLigCol = DLCol;
	#endif
		...
		#if	N_F_ESSS_ON && !defined(N_F_PA_ON)
...
void RT_PL( LightLoopContext context , LightData Plight , float3 input , out float4 PunLigCol , out float3 PuncLigDir , out float PuncSha , out float PuncLF , PositionInputs posInput , BuiltinData builtinData )
{
    ...
    float4 PLCol = float4(Plight.color * GetCurrentExposureMultiplier(), 1.0);
	#ifdef N_F_LLI_ON
		PunLigCol = clamp(PLCol,_LLI_Min,_LLI_Max);
	#else
		PunLigCol = PLCol;
	#endif
...
void RT_AL( LightLoopContext context , LightData Alight , PositionInputs posInput , float3 viewDirection , float3 normalDirection , float3 input , float4 objPos , out float4 ALigCol , out float3 ALigDir , out float ASha , out float ALF , out float ALF2 , out float3 ALCk , BuiltinData builtinData )
{
	...
	if (Alight.lightType == GPULIGHTTYPE_TUBE)
	{
		...
		if (ALF != 0.0)
		{
			...
			#ifdef N_F_LLI_ON
				float3 ALColTub = clamp(Alight.color,_LLI_Min,_LLI_Max);
			#else
				float3 ALColTub = Alight.color;
			#endif

			#if N_F_ALSL_ON
				ALigCol = float4(ALColTub * I_ltc_line( transpose(k_identity3x3) , center , axis , Alight.size.x * 0.5 ) *  GetCurrentExposureMultiplier() ,1.0);
			#else
				ALigCol = float4(ALColTub * ALF2 * GetCurrentExposureMultiplier() ,1.0);
			#endif
		}
	...
	else if (Alight.lightType == GPULIGHTTYPE_RECTANGLE)
	{
		...
		if (dot(Alight.forward, unL) < FLT_EPS)
		{
			...
			if (ALF != 0.0)
			{
				...
				#ifdef N_F_LLI_ON
					float3 ALColRec = clamp(Alight.color,_LLI_Min,_LLI_Max);
				#else
					float3 ALColRec = Alight.color;
				#endif
				...
				float3 formFactor = (float3)0.01;
					...
					ALigCol = float4( (ALColRec * ALRTLF) * GetCurrentExposureMultiplier() ,1.0);
				#else
					ALF2 = 1.0;
					...
					ALigCol = float4( (ALColRec * ALCk) * GetCurrentExposureMultiplier() ,1.0);
				#endif
