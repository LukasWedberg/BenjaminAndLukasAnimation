...
#else
			EnvLightData ELD = _WorldEnvLightDatas[envLightIdx];
#endif
...
void RT_DL_RT( LightLoopContext context , DirectionalLightData light , out float4 DirLigCol , out float3 DirLigDir , out float DirSha , out float DirLigDim , out float DirDifDim , out float DirSpecDim , PositionInputs posInput )
{
    ...
    float4 DLColRT = float4(light.color, 1.0);
	#ifdef N_F_LLI_ON
		DirLigCol = clamp(DLColRT,_LLI_Min * 2000,_LLI_Max * 2000);
	#else
		DirLigCol = DLColRT;
	#endif
...
void RT_PL_RT( LightLoopContext context , LightData Plight , out float4 PunLigCol , out float3 PuncLigDir , out float PuncSha , out float PuncLF , PositionInputs posInput )
{
	...
	//=========================
		...
		float4 PLColRT = float4(Plight.color, 1.0);
		#ifdef N_F_LLI_ON
			PunLigCol = clamp(PLColRT,_LLI_Min * 2000,_LLI_Max * 2000);
		#else
			PunLigCol = PLColRT;
		#endif
...
void RT_AL_RT( LightLoopContext context , LightData Alight , PositionInputs posInput , float3 viewDirection , float3 normalDirection , float4 objPos , out float4 ALigCol , out float3 ALigDir , out float ASha , out float ALF , out float ALF2 , out float3 ALCk )
{
	...
	if (Alight.lightType == GPULIGHTTYPE_TUBE)
	{
		...
		if (ALF != 0.0)
		{
			...
			#ifdef N_F_LLI_ON
				float3 ALColTubRT = clamp(Alight.color,_LLI_Min,_LLI_Max);
			#else
				float3 ALColTubRT = Alight.color;
			#endif

			#if N_F_ALSL_ON
				#if SHADERPASS == SHADERPASS_RAYTRACING_FORWARD
					ALigCol = float4(ALColTubRT * I_ltc_line( transpose(k_identity3x3) , center , axis , Alight.size.x * 0.5 ) *  GetCurrentExposureMultiplier() ,1.0);
					//ALigCol = float4(ALColTubRT * LTCEvaluate(P1, P2, B, k_identity3x3) *  GetCurrentExposureMultiplier() ,1.0);
				#else
					ALigCol = float4(ALColTubRT * ALF2 ,1.0);
				#endif
			#else
					ALigCol = float4(ALColTubRT * ALF2, 1.0);
			#endif
	...
	else if (Alight.lightType == GPULIGHTTYPE_RECTANGLE)
	{
		...
		if (dot(Alight.forward, unL) < FLT_EPS)
		{
			...
			if (ALF != 0.0)
			{
				...
				#ifdef N_F_LLI_ON
					float3 ALColRecRT = clamp(Alight.color,_LLI_Min,_LLI_Max);
				#else
					float3 ALColRecRT = Alight.color;
				#endif
				...
				float3 formFactor = (float3)0.01;
					...
					if ( Alight.cookieMode != COOKIEMODE_NONE )
					{
						...
						ALigCol = float4((ALColRecRT * ALRTLF), 1.0);
					...
					if ( Alight.cookieMode != COOKIEMODE_NONE )
					{
						...
						ALigCol = float4((ALColRecRT * ALCk), 1.0);
