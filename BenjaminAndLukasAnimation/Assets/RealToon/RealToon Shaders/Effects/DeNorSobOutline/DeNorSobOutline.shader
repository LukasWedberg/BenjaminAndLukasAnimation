...
Shader  "Hidden/HDRP/RealToon/Effects/DeNorSobOutline"
{
    ...
    HLSLINCLUDE
        ...
        float EdgeDetect(float2 uv, float4 input_pos_cs)
        {
            ...
            float depth = SampleClampedDepth(uv); //LoadCameraDepth(input_pos_cs.xy);
            ...
            float edgeSob = sob_fil( (_OutlineWidth + 0.49) * obj_only, uv * _ScreenSize.xy) > ((_SobelOutlineThreshold * 0.01)) ? 1 : 0;
